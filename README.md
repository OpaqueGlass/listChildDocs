...
> 当前版本：v0.2.2.1 **修复**：设置排序方式后无法获取文档的问题(fetch error)；
> 上一版本：v0.2.2 **此版本包括一些设置、显示更改！可能并不稳定** 升级至此版本，将默认迁移1次custom.js中自定义的默认数据，详见自定义设置一节。
> **新增**：快速删除其他listChildDocs挂件；*【如果正在使用文档内创建列表的模式，可考虑改为使用“目录插件”，使用此功能可帮助您删除挂件】*（设置-全局设置-最下方“重置和危险操作”）
> **新增**：自动分列（分列数设置为0）；
> **改进**：大纲支持选择开始和结束层级；
> **新增**：支持挂件内保存默认设置、部分全局设置；
> **新增**：文档内模式 创建本地文件夹`file://`URL列表；
> **改进**：额外读取`/data/storgae/listChildDocs/custom.css`，可自定义挂件内样式；

> **其他**：详见更新日志CHANGELOG.md；
> [更新日志（网页）](CHANGELOG.md) 
> 
