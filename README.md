...
> 用于[思源笔记](https://github.com/siyuan-note/siyuan)创建、更新子文档目录列表的挂件。
...
> 当前版本：v0.3.4 **修复**: 编辑器中选择文本时，无法从挂件打开文档的问题； **改进**：兼容思源3.1.11版本可自定义的默认图标；
>
> 详见更新日志CHANGELOG.md；[更新日志（网络）](CHANGELOG.md) ；

> 插入时模式为`默认`、启用自动刷新、层级1、分栏0（自动分栏）、大纲层级3。
> 
> listChildDocs 最初在2022年8月发布，感谢陪伴！[现已进入维护阶段](https://github.com/OpaqueGlass/listChildDocs/issues/49)，停止功能新增；如遇bug缺陷请反馈。
> 
...
> 请注意，`listChildDocs`是一个挂件，不是插件；挂件作为文档内容块存在，需要您手动插入到文档中（编辑器内`/`菜单->挂件）。另请参考：[挂件](siyuan://blocks/20210824201257-cy7icrc)
...
请到github仓库[新建issue](https://github.com/OpaqueGlass/listChildDocs/issues)。您也可以[在这里](https://github.com/OpaqueGlass/listChildDocs/releases)下载历史版本。
