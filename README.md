...
- 安全模式`safeMode`
  ...
  目录列表写入文档时禁用自动刷新，防止不知情时文档被更新（如果使用同步，请勿将`safeMode`设置为`false`[^4]）；

- 显示（写入）文档图标`emojiEnable`；

- 挂件内展示时根据目录长度自动更改挂件高度： `autoHeight`；

- 一并生成叶子文档的大纲目录：`showEndDocOutline`；（刷新极慢，挂件内列出时体验极差）

- 超级块分列（分栏）时在缩进位置折断：`divideColumnAtIndent`；

- 

- 写入自定义emoji图片：`customEmojiEnable`；
