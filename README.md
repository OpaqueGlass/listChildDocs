...
> 当前版本： v0.2.1 **改进**：默认获取全部子文档目录，不限制子文档数量；（思源v2.8.5及以上版本可用）**改进**：更改排序方式；（思源v2.8.7及以上版本可用）；**开发**：尝试兼容旧版Webview；**修复**：部分移动设备上文档内分列时随机块id生成错误的问题；**其他**：详见更新日志CHANGELOG.md；
