...
6. 列表写入模式：
   ...
   模式名称前有`1.`的，说明是有序列表模式，有`1.1.`的，说明是

   1. 无序列表\*

      - `默认`： （挂件内）“引用块”（无浮窗）；

      - `挂件beta`：（挂件内）“引用块”（有浮窗，不稳定）；
...
- 创建（插入）挂件时的默认参数（分列数、层级数、模式）`custom_attr`（如果写入文档，不要将auto的默认值设定为true）；
...
- 安全模式`safeMode`：默认启用。
  ...
  目录列表写入文档时禁用自动刷新，防止不知情时文档被更新导致同步冲突/覆盖（如果使用同步，请勿将`safeMode`设置为`false`[^4]）；

- 挂件内展示时根据目录长度自动更改挂件高度： `autoHeight`；默认禁用；
