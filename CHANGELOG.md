...
- 适配&修复：思源深色模式引入 color-scheme，导致挂件在深色模式下显示为白底的问题；
...
- 改进：兼容思源v3.7.0起调整界面语言标识的变动，以便插件在该版本能够正确调整挂件语言；（[REF](https://ld246.com/article/1781888259074)）

### v0.4.6 (2025年12月13日)

- 改进：兼容思源v3.4.0+以上版本对挂件宽高的调整，修复挂件展开设置项但需滚动显示的问题；

### v0.4.5 (2025年9月10日)

- 修复：预览模式下挂件未能正确载入设置的问题；
...
- 修复：导图模式折叠展开后文档名称颜色不一致的问题；
- 改进：导图模式默认关闭所有选项；
- 改进：导图模式避免双击选中文本，但导致双击放大缩小；

### v0.4.0 (2025年6月27日)

- 新增：记忆导图折叠状态；
- 改进：记忆导图缩放、平移状态的应用方式和使用场景；
- 改进：文档中模式支持动态图标；

### v0.3.7 (2025年6月19日)

- 修复：大纲为空标题时，无法显示的问题；
- 修复：文档中列表重写IAL时，部分特殊字符未转义的问题；

### v0.3.6 (2025年3月22日)

- 修复：URL+大纲标题居中时，显示html居中代码的问题；
...
- 修复：当编辑器中存在选区时，无法打开文档的问题；
- 改进：兼容思源3.1.11版本可自定义的默认图标；

### v0.3.3-rc1 （2024年10月18日）

- 改进：在挂件不可见时自动调整高度（兼容层级导航插件隐藏内容区）；
- 修复：数据库模式再二次刷新时，由于未能正确过滤已存在的块，导致思源大量警告日志的问题；
...
- 修复：“数据库”模式遇到`Unexpected end of JSON input`错误的问题（跟随思源v3.0.17接口名变更 https://github.com/siyuan-note/siyuan/issues/10201）；

### v0.3.0 (2024年5月27日)

- 改进&修复：更严格地执行安全模式，安全模式启用状态下，页签切换自动刷新也禁用，需要写文档的模式全部禁用；
- 新增：（测试中）新模式，数据库；

### v0.2.8 (2024年5月22日)

- 改进：本地文件夹目录模式支持不同系统选择不同路径；
