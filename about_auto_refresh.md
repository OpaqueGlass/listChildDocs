# 文档中自动刷新、自动插入助手风险说明和使用建议

> 更新于listChildDocs版本v0.1.1-beta4

## 可能导致的问题

1. **同步覆盖**。如果执行自动刷新时并没有完成同步，那么设备的旧文档将覆盖云端的新文档，导致其他设备的编辑丢失。
...
> 注意：⚠可能存在未被发现的bug。由于自动插入助手对全局生效，请谨慎使用。如果您发现了使用问题，请提交issue反馈，谢谢。
...
满足以下情况之一，有概率重复插入挂件或链接，不建议启用自动插入助手：
...
在`config.js`（或`custom.js`）中修改配置`helperSettings`，具体配置项见`config.js`文件内的说明，示例见本文档中配置示例部分。
...
1. 由自动插入助手插入的子文档链接/引用所在段落块将会被设定备注，对应的段落块由自动插入助手管理：

    - 当启用自动删除时（removeLinkEnable），检测到子文档删除后将删除该段落块；
    - 当启用自动修改时（renameLinkEnable），检测到子文档重命名后将更新该段落块；
    
    因此，**在开启了上述选项时，请不要在对应段落块内编辑，否则您的修改可能丢失**。建议使用鼠标以块为单位做修改，不更改段落块里的内容。
...
1. 向父文档中自动插入siyuan://URL；
   
   - 插入到文档末尾；
   - 文档删除时移除链接所在段落块（对应`removeLinkEnable: true`）；
   - 文档重命名时更新所在段落块（对应`renameLinkEnable: true`）；

    ```javascript
    // 配置在custom.js中，详见README.md 自定义说明
    export const config = {
        token: "",
        /*其他挂件或主题的设置（如果有）*/
        listChildDocs: {
...
2. 向父文档末尾自动插入引用块：
   ...
   - 插入到文档末尾；
   - 文档删除时移除引用块所在段落块（对应`removeLinkEnable: true`）；
