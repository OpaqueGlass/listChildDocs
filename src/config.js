...
let zh_CN = {
    ...
    removeCurrentDistinctConfim: "确定删除当前挂件独立设置吗？<br/>请注意：1.删除后，当前挂件独立设置将跟随全局，直到下次保存独立设置。<br/>2. 由当前挂件创建的子文档列表也将被一并删除",
...
let en_US = {//先当他不存在 We don't fully support English yet.
    ...
    removeCurrentDistinctConfim: "Are you sure you want to delete the current widget independent settings? <br/>Note: 1. After deletion, the current widget inpendent settings will follow the global until the next time you save settings. <br/>2. The child docs list created by the widget will also be deleted ",
